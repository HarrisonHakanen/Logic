...
public interface UsuarioRepository extends JpaRepository<Usuario,Long> {
	...
	@Query(value = "SELECT * FROM usuario_tbl WHERE senha = :senha AND usuario = :usuario", nativeQuery = true)
    public List<Usuario> login(@Param("senha") String senha, @Param("usuario") String usuario);
	
	@Query(value = "SELECT * FROM usuario_tbl WHERE usuario = :usuario", nativeQuery = true)
